                ...
                if record.type_id.is_cantonera == True:
                    ...
                    if record.referencia_id.longitud < 200:
                        ...
                        #Columnas
                            ...
                            undColumna = int(((ancho_pallet - 14) / 4 - 0.7071 * mediaAlas) / (record.referencia_id.grosor_2 * 1.53))
                            paquetesColumna = 2
    ...
    def _get_valores(self):
        for record in self:
            ...
            if record.referencia_cliente_id:
                ...
                #Cantonera
                if record.type_id.is_cantonera == True:
                    alas = record.referencia_cliente_id.ala_1 + record.referencia_cliente_id.ala_2
                    if record.referencia_cliente_id.longitud < 300:
                        sierra = True
            ...
            if record.partner_id:
                for pedido in record.partner_id.sale_order_ids:
                    for linea in pedido.order_line:
                        if linea.oferta_id.id == record.id:
                            peso = peso + linea.peso_neto * linea.num_pallets
                            if fecha == None:
